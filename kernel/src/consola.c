...
void ejecutarInstrucciones(argsPlanificacion* args, char* consola, int* contadorProcesos)
{
    ...
    //? Por ahora hasta que funcione como deberia lo dejo asi, despues lo comento y lo paso a funciones
    switch(instruccion)
    {
...
void aumentarGrado(argsPlanificacion* args, int operando)
{
    ...
    log_info(args->logger, "Aumento en: %d", iteraciones);  //?Por ahora lo tengo para testear nomas
...
void disminuirGrado(argsPlanificacion* args, int operando)
{
    ...
    log_info(args->logger, "Disminuyo en: %d", iteraciones);  //?Por ahora lo tengo para testear nomas
...
//?Si alguno se le ocurre una forma para achicar la zona critica se lo agradezo
